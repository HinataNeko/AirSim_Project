...
class CNNtoRNN(nn.Module):
    def __init__(self, cnn_hidden=128, cnn_output_features=128, rnn_hidden_size=128, output_size=4):
        ...
        self.rnn = nn.LSTM(input_size=self.cnn_output_features, hidden_size=self.rnn_hidden_size,
    ...
    def _reset_hidden(self):
        self.hidden = (torch.zeros(self.rnn_layers, 1, self.rnn_hidden_size).to(self.device),
                       torch.zeros(self.rnn_layers, 1, self.rnn_hidden_size).to(self.device))
    ...
    def forward(self, img_frames):  # (batch_size, frames, channels, height, width)
        ...
        c_0 = torch.zeros(self.rnn_layers, current_batch_size, self.rnn_hidden_size).to(self.device)

        # rnn_output: (batch_size, frames, num_directions * hidden_size)
        rnn_output, _ = self.rnn(cnn_output, (h_0, c_0))  # rnn_output: (batch_size, frames, hidden_size)

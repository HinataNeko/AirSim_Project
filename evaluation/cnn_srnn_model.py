...
class Model:
    def __init__(self, load_dataset=True):
        ...
        # cnn_total = sum([param.nelement() for param in self.model.cnn.parameters()])
        # print("Number of parameter: %.2fK" % (cnn_total / 1e3))
        # rnn_total = sum([param.nelement() for param in self.model.srnn.parameters()])
        # print("Number of parameter: %.2fK" % (rnn_total / 1e3))
        # mlp_total = sum([param.nelement() for param in self.model.output_layer.parameters()])
        # print("Number of parameter: %.2fK" % (mlp_total / 1e3))
    ...
    def plot_one_neuron_v_s(self, rgb_img, transform=True):
        ...
        fig_size = (12, 8)
        dpi = 100
        visualizing.plot_2d_heatmap(array=v_records[0][:, 0].cpu().numpy(), title='membrane potentials',
                                    xlabel='simulating step', ylabel='neuron index', int_x_ticks=True, x_max=self.T,
                                    figsize=fig_size, dpi=dpi)
        visualizing.plot_1d_spikes(spikes=spike_records[0][:, 0].cpu().numpy(), title='membrane sotentials',
...
if __name__ == '__main__':
    ...
    # input_video, _ = next(iter(base_model.test_dataloader))

...
class DroneEnvWrapper:
    ...
    def step(self, action):
        ...
        if len(detection) > 0:
            ...
            self.state_stack.append(np.array(self.target_xywh))
            ...
            self.state_stack.pop(0)
            self.state_stack.append(np.array([-1., -1., 0., 0.]))
            done = True
    ...
    def reset(self):
        ...
        if len(detection) > 0:
            self.target_xywh = self.get_target_xywh(detection[0])
            self.state_stack = [np.array(self.target_xywh) for _ in range(self.state_keep_n)]
            state = np.array(self.state_stack)
        else:
            state = None
            print("Reset Error")

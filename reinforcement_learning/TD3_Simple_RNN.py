...
from EnvWrapper_Simple_RNN import DroneEnvWrapper
...
class QNetwork(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hidden_dim, init_w=3e-3):
        ...
        self.rnn = nn.LSTM(self.state_dim, self.hidden_dim, self.rnn_layers, batch_first=True)
    ...
    def forward(self, state, action):
        ...
        h_0 = torch.zeros(self.rnn_layers, current_batch_size, self.hidden_dim).to(self.device)
        c_0 = torch.zeros(self.rnn_layers, current_batch_size, self.hidden_dim).to(self.device)
        rnn_output, _ = self.rnn(state, (h_0, c_0))  # rnn_output: (batch_size, time_seq, hidden_size)
...
class GymEnvWrapper:
    def __init__(self):
        import gym
        self.env = gym.make('Pendulum-v1', render_mode='human').unwrapped

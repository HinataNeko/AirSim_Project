...
class LIFRNN(nn.Module):
    def __init__(self, n_inp, n_hid, n_out):
        ...
        self.n_hid = n_hid
        self.n_inp = n_inp
        self.i2h = tdLinear(n_inp + n_hid, n_hid, bias=True, bn=tdBatchNorm(n_hid), spike=LIFSpike())
        self.readout = tdLinear(n_hid, n_out, bias=True, bn=tdBatchNorm(n_out), spike=LIFSpike())

    def forward(self, x):
        ## initialize hidden states
        nsteps = x.shape[-1]
        out = torch.zeros([x.size(0), self.n_hid, x.size(2)], device=x.device)
        u = torch.zeros(x.shape[:-1], device=x.device)
        ...
        # for step in range(nsteps):
        #     print()
        #     out = torch.cat((x[step], out[step]), 1)
        #     print(out.size())
        #     # u, out[step] = self.cell(u, , )
        # return out

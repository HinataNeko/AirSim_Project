...
class Drone:
    ...
    def _video_stream(self, client):
        ...
        while self.is_connected:
            ...
            salient_map = self.model.get_salient_map(img_rgb)

            if self.is_navigating:  # 正在导航
                if self.navigation_start_sequence:
                    controls = self.model.predict(img_rgb, True)
                    self.navigation_start_sequence = False
                else:
                    controls = self.model.predict(img_rgb, False)
            ...
            cv2.imshow('Salient Map', cv2.cvtColor(salient_map, cv2.COLOR_BGR2RGB))
            cv2.waitKey(1)
